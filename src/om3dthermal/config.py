...
def _build_legacy_horizontal(geometry: dict, stacks: dict) -> dict:
    ...
    When the user configures an HBM sub-template (``stacks.hbm``
    with ``base/dram/top``), the memory zone's ``reference_stack``
    is the legacy ``hbm_12hi`` template; each
    ``geometry.hbm.centers`` entry becomes a column. The optional
    ``thermal_silicon`` block also becomes a column with the
    renamed stack key.
...
def _extract_material_names(stacks: dict) -> set[str]:
    """Walk the compact ``stacks`` block and collect every
    material name referenced. Used as a fallback for the
    memory-zone background material in toy configs that do not
    declare ``Mold`` explicitly."""
    out: set[str] = set()
