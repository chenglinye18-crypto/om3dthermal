...
Read sensing and row/column selection now share a **0.04 pJ/payload-bit**
budget; write selection retains 32 pJ per row/column service. Physical event
counts and performance are unchanged. Earlier energy/placement run directories
retain their historical coefficients and are not regenerated by this update.

No-NMP read-bandwidth thermal reclosure (M3D only; HBM curves are read from
historical results):

```powershell
python -m om3dthermal thermal-sensitivity --read-peripheral-reclosure --output-dir runs/no_nmp_geometry_sensitivity_v2
```

This requires compatible existing 100/300 µm caches (`--thin-cache` and
`--thick-cache` override their paths), validates the power-independent physical
signature, and reuses both the CPU setup and GPU operator/Jacobi data. `--resume`
retains completed points only when energy/path inputs and thermal physics match.
The prescribed bandwidth grid is not an achievable-throughput claim. The old
sweep used 0.855261 pJ/bit, including a 0.001458 pJ/bit aggregate global-control
term, rather than the later 0.45 pJ/bit SA/selector budget. New physical read
accounting is 0.893802 pJ/bit; comparison must use that actual old sweep basis.
This closes only architecture-level No-NMP stress, not workload-specific NMP.
