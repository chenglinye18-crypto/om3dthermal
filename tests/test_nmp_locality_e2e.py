...
def test_nominal_resident_and_active_weight_closures_are_distinct(inputs):
...
def test_av_partial_is_resolved_per_layer_request(inputs):
    ...
    for unit in units:
        if unit.operator_type=="ATTENTION_AV":
            ownership.append((0,1) if unit.request_id==0 else (1,2,3))
        else:
            ownership.append((0,))
    layers=attention_boundary_by_layer(units,tuple(ownership))
    expected=w.n_layers*(2+3)*w.d_model*4
    assert sum(row["partial_bytes"] for row in layers.values())==expected
    assert all(row["partial_bytes"]==(2+3)*w.d_model*4 for row in layers.values())
