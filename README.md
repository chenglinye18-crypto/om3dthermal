...
The current DAC experiment entry points and baseline policy are documented in
[`docs/benchmarks/experiment_matrix.md`](docs/benchmarks/experiment_matrix.md).
The canonical thermal regression reference and comparison limits are in
...
baseline. Retired configurations are retained only as test fixtures and
provenance, and are excluded from current paper tables and run examples.

Geometry compatibility commands remain available. This fixed-power 2x1
fixture illustrates the geometry CLI; it is not the analytical DAC power
working point:

```powershell
python -m om3dthermal.cli build configs\legacy\exp_conv_2x1_g300_m160.yaml --out runs\hbm12_geometry
```
...
## Conventional HBM configuration and provenance

`configs/cases/conventional_hbm_2x1.yaml` defines the sole Conventional DAC
baseline. Its two continuous thermal groups represent four physical stack
equivalents in total; the layout name does not mean two physical HBM stacks.
The canonical case and architecture descriptor retain geometry and power
provenance. The measured source power maps are unavailable, so this is not a
strict reproduction of a literature temperature.

Historical benchmark configuration notes are retained under `docs/archive/`
for provenance only. Their fixed-power results are not current DAC results.
...
`python -m om3dthermal.cli conductance configs/legacy/exp_conv_2x1_g300_m160.yaml --out runs/hbm12_conductance`
...
- `conductance_edges.csv` — only when `--write-conductance-csv` is
  passed (NPZ is preferred for large edge tables).
...
### Conventional HBM regression reference

The only Conventional DAC baseline is
`configs/cases/conventional_hbm_2x1.yaml`. At canonical tolerances its
FP64 matrix-free GPU-PCG regression anchors are 859,596 cells,
2,531,340 edges, approximately 355.58349 W package input and 81.93349 degC
Tmax. Use the formal experiment command above for production evaluation.
See [the current thermal reference](docs/benchmarks/thermal_results_overview.md)
for the result provenance and comparison limits.
